...
def test_like_and_notification(page: Page, social_media_url: str):
    ...
    for user in users:
        ...
        text = page.locator(f"xpath=//h6[contains(text(), '{user}')]/../..//button/following-sibling::p").inner_text()
        ...
        page.locator(f"xpath=//h6[contains(text(), '{user}')]/../..//button").click()

        expect(page.locator(f"xpath=//h6[contains(text(), '{user}')]/../..//button/following-sibling::p")).to_have_text(f"{str(initial_likes + 1)} likes")
        ...
        expect(page.locator("xpath=//h6[contains(text(), 'Notifications')]/following-sibling::*//p").filter(has_text=f"You liked {user}'s post")).to_be_visible()
